...
if __name__ == '__main__':
    ...
    if dataset_name == "VGGFace":
        ...
        testset = torchvision.datasets.FashionMNIST('./dataset/MNIST', train=False, transform=MNIST_transforms, download=True)
        test_images = []
        test_labels = []
        for i in range(len(testset)):
            img = testset[i][0]
        ...
        print('./dataset/'+dataset_name+'/test/')
    ...
    if model_name == "resnets":
        ...
        test_loader_poison = DataLoader(TensorDatasetImg(test_images,test_labels,transform=imagenet_transforms,mode='test',test_poisoned='True',transform_name='imagenet_transforms_test'),
                                ...
                                num_workers=4,
                                pin_memory=True)
    elif model_name == "MNISTNN":
        train_loader = DataLoader(TensorDatasetImg(train_images, train_labels, transform=MNIST_transforms),
                                  shuffle=True,
                                  batch_size=batch_size,
                                  num_workers=4,
                                  pin_memory=True)

        test_loader = DataLoader(TensorDatasetImg(test_images, test_labels, transform=MNIST_transforms, mode='test',
                                                  test_poisoned='False'),
                                 shuffle=False,
                                 batch_size=64,
                                 num_workers=4,
        ...
        test_loader_poison = DataLoader(
            TensorDatasetImg(test_images, test_labels, transform=MNIST_transforms, mode='test', test_poisoned='True'),
            shuffle=False,
            batch_size=64,
            num_workers=4,
            pin_memory=True)
